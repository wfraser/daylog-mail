...
impl Database {
    ...
    pub fn add_entry(&mut self, username: &str, date: &str, body: &str) -> Result<(), failure::Error> {
        ...
        if let Err(rusqlite::Error::SqliteFailure(rusqlite::ffi::Error {
            ...
            let mut update_body: String = tx.query_row_named(
                "SELECT body FROM entries WHERE username = :username AND date = :date",
                rusqlite::named_params!{":username": username, ":date": date},
                |row| row.get(0))?;
            update_body.push('\n');
            update_body +=  body;
            tx.execute_named("UPDATE entries SET body = :body", named_params!{":body": update_body})
